...
classdef Naive_Bayes < handle
    ...
    methods
        function obj=Naive_Bayes(data,sizeFilter,isTest)
            ...
            if isTest==1
                ...
                obj.bloonFilter=obj.bloonFilter.initializeDataStructure(sizeFilter);
                optimalK=round(sizeFilter * log(2) / length(data.z)); % Optimal k formula
                k = max(10, 2 * optimalK);
                obj.sizeFilter=sizeFilter;
                obj.bloonFilter=obj.bloonFilter.defineParams(k);
            end
        end
        ...
        function h = string2hash(obj,str)
            h = double(0);
            for i = 1:length(str)
                h = mod(h * 31 + double(str(i)), 2^31 - 1);
            end
        end
        ...
        function obj=buildFeature(obj)
            ...
            if obj.isTest==1
                for idx=1:height(obj.Data)
                    key = sprintf('%.6f_%.6f_%.6f_%.6f_%.6f_%.6f_%.6f', ...
                        obj.Data.redshift(idx), ...
                        obj.Data.u_g(idx), ...
                        obj.Data.g_r(idx), ...
                        obj.Data.r_i(idx), ...
                        obj.Data.i_z(idx), ...
                        obj.Data.velDisp(idx), ...
                        obj.Data.snMedian_r(idx));
                    %adding the values on the data set to the bloon filter
                    obj.bloonFilter=obj.bloonFilter.addElement(obj.string2hash(key));
                end
            end
        end
        ...
        function [class]=estimateClass(obj,tuple,useLog)
            ...
            if obj.isTest==1
                key = sprintf('%.6f_%.6f_%.6f_%.6f_%.6f_%.6f_%.6f', ...
                    featureTuple.redshift, ...
                    featureTuple.u_g, ...
                    featureTuple.g_r, ...
                    featureTuple.r_i, ...
                    featureTuple.i_z, ...
                    featureTuple.velDisp, ...
                    featureTuple.snMedian_r);

                if obj.BelongsToTranningSet(key)==true
                    classes = obj.Data( ...
                        obj.Data.redshift == featureTuple.redshift & ...
                        obj.Data.u_g == featureTuple.u_g & ...
                        obj.Data.g_r == featureTuple.g_r & ...
                        obj.Data.r_i == featureTuple.r_i & ...
                        obj.Data.i_z == featureTuple.i_z & ...
                        obj.Data.velDisp == featureTuple.velDisp & ...
                        obj.Data.snMedian_r == featureTuple.snMedian_r, :).class;
                    class=classes;
                    if (~isempty(class))
                        return;
                    end
        ...
        function [isFromTrainninSet]=BelongsToTranningSet(obj,key)
            isFromTrainninSet=obj.bloonFilter.isElementInFilter(obj.string2hash(key));
        end
    end
end
